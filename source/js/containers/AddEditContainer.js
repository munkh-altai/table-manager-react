...
class AddEditContainer extends Component {
    ...
    updateForm() {
        ...
        if (foundError === false){
            this.setState({sending: true});
            update(FD, this.props.translateFormControls, this.props.match.params.id, this.props.subItems, multiItems).done((data)=> {
                localStorage.setItem("addEditFromStatus", "ended");
                this.setState({sending: false});
    ...
    componentWillMount() {
        ...
        if (this.props.match.params.id) {
            ...
            edit(this.props.match.params.id).then((data)=> {
                if (data.length >= 1){
    ...
    render() {
        ...
        const edit_parent_id = this.props.match.params.id ? this.props.match.params.id : false
